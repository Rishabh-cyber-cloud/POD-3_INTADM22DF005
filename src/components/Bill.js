...
export default class Bill extends Component {
  constructor() {
    ...
    this.state = {
      ...
      RoomBill: 0,
      Total: 0,
      Logged_in:JSON.parse(localStorage.getItem('Login')) // value get login from local storage
      // JSON.parse() convert data (string format from server) to js obj 
    }
  }
  handleLogout = (e) => {
    ...
    localStorage.setItem('Login', JSON.stringify(login)); // when data sends to servers it should be in string, convert js obj to string
    // this.setState()
  ...
  handleBill = (e) => { // Arrow Function
      console.log(this.state.Logged_in) // state is built in object in components which stores key-value pair
      let data = {
      ...
      {headers:{
        ...
        if (data.data.isSuccess) {
          this.setState({BillId:data.data.billId,RoomBill:data.data.roomBill,MessBill:data.data.messBill,Total:parseInt(data.data.roomBill)+parseInt(data.data.messBill)}) //change components value in front end
